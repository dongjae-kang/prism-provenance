...
**Status**: PRD locked (2026-05-06, hook layer integrated)
**Demo hard-deadline**: 2026-05-09 ~ 05-10 (working demo, design + hook elements integrated)
**Implementation wrap-up**: 2026-05-16 ~ 05-17 (QA + final polish)
**Launch target**: 2026-05-30 (선거 D-4)
**First application**: 2026-06-03 지방선거 (Track 1.5)
**Relation to v2**: v2.0 (`PRD-v2.md`)의 운영 layer는 그대로 carry. v3은 (a) 5/5 [[김재훈]] 콜에서 결정된 데이터 소스·윤리·운영 변경, (b) 5/5 [[김현미]] 1차 인계 미팅 피드백을 반영한 self-positioning 강화, (c) 5/6 hook layer brainstorm session에서 결정된 2030 secondary target felt pull layer — 세 축의 통합본.
...
| Saetbyeol 합류 시점 | "5/4 합류 확정" | **5/11 합류** |
| 샘 (Sam Lee, EPFL Computer Science) | 미언급 | **합류 완료, 이번 주부터 함께 작업** |
...
| 6번째 멤버 | TBD | **샘(Sam Lee, EPFL CS) 합류로 6명 확정** (4명 + 샘 + 5/11 Saetbyeol 합류 예정) |
| Hook layer | 없음 | **§4.5 Hook & Felt Pull layer 신규** — 정치 관심 적은 2030 secondary target felt pull. 5개 hook (오늘의 흐름 시각화 / N일째 카운터 / 분광 메타포 / 사이트 hero lede / Weekly Flow 리더보드) + 카드 hero 수치 trajectory + 카톡 공유 한 줄 hook + 정정 요청 채널 |
...
### 4.1 카드 form factor (v3 메타 + hook layer)
*Card Form Factor (Meta Lines + Hook Layer)*

PRISM의 사용자 facing primary unit은 **카드**. 카드는 모바일 우선 디자인이며, 카톡·X 공유에 최적화된다. v3은 정치 관심 적은 2030 secondary target의 felt pull을 높이기 위해 카드 hero에 hook layer를 추가한다.

**카드 구성요소 (필수 12개 + 조건부 2개):**

1. **이슈 라벨** (예: "주거", "노동", "AI 규제") — 진입 식별자
2. **N일째 추적 중 카운터 (v3 hook)** — 카드 좌상단 작은 라벨 (예: "📍 14일째 추적 중"). 흐름 매체 정체성을 카드마다 visible.
3. **카드 제목 — 수치 trajectory 포함 (v3 hook)** — 가능하면 변형 trajectory를 제목에 박음 (예: "청년 무주택 통계: 35% → 50% → 60% (3일간)"). Scroll에서 즉시 인지되는 hook. Trajectory가 명확히 추출 안 되는 카드는 "이슈 + 한 줄 핵심"의 standard 제목.
4. **카드 hero 시각 — 수치 trajectory 시각화 (v3 hook)** — 제목 아래 큰 글자 또는 mini chart로 변형 폭 시각화. Recognition 강화.
5. **본문 (200-400자)** — 어떤 클레임이 어떻게 돌고 있는지 factual 기술
6. **클레임 병렬 제시** — 정치 YouTube에서 발견된 2-4개 클레임을 발화자·발화일·발언과 함께 나열
7. **시간 순서 메타** — 각 클레임에 "가장 먼저 등장 / 24시간 후 / 익일 변형 / 주말 동안 확산" 등 시간 축 라벨
8. **출처 성격 메타** — 각 클레임에 "정치인 직접 발언 / 언론 인용 / 익명 커뮤니티 유출 / 2차 가공 채널" 등 source-type 라벨
9. **Canonical source 병렬 제시** — 관련 통계청·부처·후보 공식 발표·data.go.kr dataset 등 1-3개 출처
10. **출처 링크 (전부)** — 각 클레임의 영상 링크 + 각 canonical source의 원본 링크
11. **Provenance footer** — "PRISM은 발언을 분류하지 않습니다. 출처를 보여드립니다. 사람이 검수했습니다."
12. **공유 버튼** — 카톡, X, 링크 복사

**조건부 (검토 후 채택):**
- (a) **영상 썸네일 (v3 conditional hook)** — 각 클레임 옆에 YouTube 썸네일 표시. "어, 이 영상 본 적 있다"는 recognition moment. 저작권·캐싱 부담은 김재훈 검토 후 결정.
- (b) **분광 아이콘 (v3 conditional hook)** — 카드 좌상단 작은 PRISM 분광 SVG. 디자인 quality bar 통과 시만 채택 — quality 미달 시 drop.

**카드 form factor 예시 (v3, hook layer 포함):**
...
**카드 hero 영역 design intent:**
- 좌상단: N일째 카운터 (메타) + 분광 아이콘 (조건부, design 통과 시)
- 중앙 상단: 수치 trajectory가 한눈에 들어오는 제목 + mini-chart
- 이 hero가 카드 list scroll에서 즉시 인식되는 hook 역할 (recognition + instinctive interest)
...
### 4.4 카톡 공유 최적화 (v3 — 한 줄 hook 추가)
*KakaoTalk Sharing Optimization*

한국에서 정치 콘텐츠 viral의 primary path는 카톡 공유. PRISM은 카톡 공유 mechanic을 first-class citizen으로 design.

**카톡 미리보기 최적화:**
- Open Graph 메타데이터: 카드 제목, **공유 한 줄 hook (v3 신규)**, 이슈 라벨 색상 thumbnail
- 미리보기에서 핵심 정보 (제목 + 이슈 + "출처 X개") 즉시 visible
- "PRISM" 로고 작게 워터마크 (브랜드 인지)

**공유 한 줄 hook (v3 신규):**
- 큐레이션 워크플로우에 "공유 한 줄" 항목 추가 (큐레이터 작성)
- 본문과 별개로, 카톡 미리보기 description 슬롯에 들어가는 instinctive interest 한 줄
- §4.2 Show-don't-classify 준수 (factual only, 평가 표현 금지)
- 좋은 예: "청년 무주택 통계가 35%·38%·50%·60% 네 가지로 등장했습니다."
- 좋은 예: "이 클레임은 14일째 변형 중입니다."
- 나쁜 예: "충격! 통계 왜곡 사례" (평가 표현 X)
- 나쁜 예: "후보 X의 거짓말" (분류 X)
...
### 4.5 Hook & Felt Pull layer (v3 신규)
*Hook & Felt Pull Layer (New in v3)*

PRD-v3까지의 모든 product 결정은 신뢰·정체성 lock에 집중되었다. v3 hook layer는 그 위에 정치 관심 적은 2030 secondary target의 felt pull을 의도적으로 높이는 layer다.

**핵심 원칙:** Hook은 "재미"가 아니라 *substance의 surprise + 자기 정체성 신호*. PRISM 정체성 (흐름 매체 + 신뢰)과 같은 axis 위에 있는 hook만 채택한다. 캐릭터 voice·위트·풍자는 §4.1 톤 가이드라인에 따라 여전히 금지.

#### 4.5.1 사이트 hero — "오늘의 흐름" 시각화 (Radial Spread)

사이트 진입 즉시 사용자가 보는 첫 시각 element. 카드 list가 아니라 **흐름 자체**가 첫 인상.

**시각 형태 (B안 — Radial Spread, A안 Sankey diagram은 데모 단계 비교):**
- 중심 노드: 그날의 main canonical claim (예: 통계청 청년 무주택 35%)
- 바깥 노드: 정치 YouTube에서 변형된 클레임들 (38%, 50%, 60%)
- 중심 → 바깥 edge: 변형 trajectory. 두께 = 확산 정도 또는 변형 폭
- Hover/click → 해당 카드로 이동
- 시각 메타포: PRISM 분광 — 한 줄기 빛(canonical source)이 여러 색(변형된 클레임)으로 분광

**색상 design 가이드 (5/6 갱신):**
- **무지개 색 가능** — 빨강·파랑이 모두 포함되어 한쪽 진영 강조가 아닌 *diversify된* visual로 작동 가능 (Jack 5/6 reframing).
- 후보 색 design: (a) 무지개·multi-color — diversify 메타포 + 분광 시각 정체성과 일치, (b) Monochrome — 흑백 + 회색 그라데이션, (c) 단일 cool tone — 청록·보라·남색 조합, (d) 출처 성격별 색상 mapping (정치인 직접 / 언론 인용 / 2차 가공)
- **Design 단계에서 검토할 점**:
  - 한국 LGBTQ 무지개 깃발 함의 — 정치 매체에서 의도와 무관하게 정치적 함의로 읽힐 가능성. 채도·톤 조절로 brand identity와 분리 가능 여부.
  - 정보 시각화 noise — 색상이 출처 성격 등 *위계*를 매핑할 때 무지개는 패턴 읽기를 어렵게 함. 위계 매핑 시 색상 ordering 신중.
  - 신뢰 매체 톤 — 너무 colorful하면 trustworthy positioning 약화. 채도·면적 조절.
- 김현미 + Saetbyeol + 샘 디자인 단계에서 final 결정.

**A안 (Sankey diagram) 검토:**
- 좌측: canonical sources (통계청·국토부·data.go.kr 등), 우측: 정치 YouTube 클레임들
- 중간 흐름이 두께로 표현
- 강점: 출처 → 클레임 mapping이 더 명시적
- 약점: 모바일에서 답답함, 분광 메타포와 fit 약함
- **결정**: Claude Code 데모에서 두 안 prototype 후 비교 — quality 우월 안 final 채택

**🔴 Pre-launch backfill 필수 (§6.1 워크플로우에 명시):**
시각화는 데이터가 있어야 의미. Launch (5/30) 전 1주 (5/24~) 시드 클레임 데이터 백필 작업 필수. 그래야 launch day "추적 중인 클레임 24개, 가장 오래된 것 21일째" 같은 hero가 빈약하지 않음.

#### 4.5.2 N일째 추적 카운터 (3-layer 통합)

흐름 매체 정체성을 매일 visible한 metric으로 surface.

**Site-level (사이트 홈):**
- 헤더에 "지금 PRISM이 추적 중인 클레임 24개. 가장 오래된 것 21일째"
- 매일 자동 갱신
- Site visit ritual의 anchor

**Card-level (각 카드 좌상단):**
- 작은 라벨: "📍 14일째 추적 중"
- 카드 list scroll에서 카운터 비교 가능 → "이 클레임이 가장 오래 변형 중"이 즉시 인지

**Weekly Flow-level (토요일 카드):**
- Weekly Flow의 핵심 지표: "이 클레임 N일째 N차 변형"
- 한 주의 흐름을 N일 metric으로 압축

**🔴 Backend schema 명시 (재훈에게):**
- 클레임 ID + 시작일 추적 schema가 데이터 모델에 박혀야 함
- 클레임 ID 단위 변형 history (N차 변형) 기록 필수
- 5/11 kick-off에서 schema 합의

**🔴 Launch 전 backfill 필요:**
"1일째"부터 시작하면 hook 약함. Launch 시점에 "21일째 추적 중" 같은 카운터가 보이려면 launch 전 backfill 필수. §4.5.1과 함께 5/24~ backfill 작업.

#### 4.5.3 PRISM 분광 메타포 — Design Language (조건부 채택)

**채택 조건 (Jack lock):** 디자인 quality bar 통과 시만 채택. 자칫 완성도 떨어지거나 문제 소지 있으면 아예 안 함이 낫다 (5/6 Jack 결정).

**활용 layer (quality 통과 가정):**
1. **Logo** — 빛이 prism을 통과해 분광되는 단순화 SVG. 색상 design constraint (§4.5.1) 준수.
2. **사이트 hero visual** — §4.5.1 radial spread = prism 분광 (두 hook을 하나의 artifact로 통합)
3. **카드 hero corner** — 각 카드 좌상단 작은 분광 아이콘 (조건부 채택)
4. **"How We Work" 페이지 (§4.6)** — "우리는 정보를 분광한다, 분류하지 않는다" 헤더
5. **Brand voice** — "분광" "스펙트럼" 단어를 의도적으로 사용 (예: "이번 주의 스펙트럼")

**Drop 조건:**
- 디자인 quality 미달 → 분광 메타포 전체 drop, 단순 typography 기반 brand identity로 fallback
- LGBTQ 함의 또는 정보 시각화 noise 검토 실패 → drop
- 모바일에서 분광 visual이 작동 안 함 → drop

#### 4.5.4 사이트 hero 한 줄 lede (매일 갱신, factual only)

신문 1면 lede 효과. 사이트 진입 시 큰 글자 한 줄.

- 큐레이터 작성 (일일 워크플로우에 항목 추가)
- §4.2 Show-don't-classify 준수 (평가 표현 금지)
- 좋은 예: "오늘 청년 무주택 통계 인용이 네 가지 형태로 등장했습니다."
- 좋은 예: "이번 주 가장 활발히 변형된 클레임은 청년 무주택 통계 (4차 변형)."
- 나쁜 예: "오늘의 충격 거짓말 클레임" (평가)

#### 4.5.5 Weekly Flow 리더보드 (사실 묘사 only)

토요일 Weekly Flow를 "이번 주 흐름 ranking" 형식으로 surface.

- 좋은 예: "이번 주 가장 멀리 간 클레임 Top 3" (확산 측정)
- 좋은 예: "이번 주 가장 빠르게 변형된 클레임" (시간 측정)
- 좋은 예: "이번 주 가장 많이 인용된 canonical source" (출처 ranking)
- 나쁜 예: "이번 주 가장 왜곡된 클레임" (평가 — 분류)
- 나쁜 예: "이번 주 가장 거짓에 가까운 발언" (분류)

Ranking은 **사실 metric**으로만 (확산/속도/시간/인용), **평가**로는 절대 X.

### 4.6 Process Transparency Disclosure (v3, 기존 §4.5)
*Process Transparency Disclosure*
...
### 4.7 정정 요청 채널 (v3 신규)
*Correction Request Channel (New in v3)*

사용자가 PRISM 카드에서 사실 오류를 발견했을 때 정정을 요청할 수 있는 form. v3 hook 검토 단계에서 "제보" (#7)는 운영 부담으로 거절되었으나, 그 input loop의 가치 — 사용자 참여 + 신뢰 강화 — 를 정정 요청 형태로 보존.

**구조:**
- 카드 footer에 작은 "정정 요청" 링크
- Form 항목: (1) 카드 URL (2) 정정이 필요한 부분 (3) 근거 출처 link (4) 제출자 연락처 (선택)
- 24시간 내 큐레이터 검토 + 응답
- 검증된 정정은 §C.1 retraction protocol에 따라 카드 revision (revision history visible)

**Why 정정 요청 (제보 X):**
- 제보 (input)는 PRISM이 외부에서 들어오는 정보를 처리해 다른 매체에 전달하는 process — Track 1.5 scope 외, 운영 부담 큼
- 정정 (correction)은 PRISM 자체 카드의 사실 오류 fix — 24h retraction protocol과 직접 연결, 부담 작음
- 신뢰 loop: "사람이 검수했다 + 사용자도 정정 가능" — Process Transparency Disclosure (§4.6)와 같은 axis

**Why 2030 hook과 연결되는가:**
정정 요청 form 자체는 hook이 아니지만, "이 매체는 사용자 참여로 더 정확해진다"는 신호가 PRISM의 신뢰 매체 정체성을 강화. 2030 사용자에게 "내가 본 매체가 잘못된 부분을 fix할 수 있다"는 agency 제공.
...
**일일 워크플로우 (v3 — hook 항목 추가):**

```
오전 8:00  큐레이터 lead (Jack) data.go.kr + YouTube monitoring 결과 review
오전 8:30  AI assist로 trending claims 자동 추출 → 큐레이터 검토 큐
           + 클레임별 N일째 카운터 자동 갱신 (§4.5.2)
오전 9:00  큐레이터 lead가 그날 카드 후보 10-15개 선정
오전 9:30  AI assist로 카드 초안 자동 작성:
           - claim + canonical source lookup
           - 시간/출처 메타 자동 분류
           - 수치 trajectory 추출 (제목 + hero용, §4.1)
오전 10:00 큐레이터 lead가 5장 final 선정 + 인간미·톤 가다듬기
           + 카드별 "공유 한 줄 hook" 작성 (§4.4)
           + 사이트 hero 한 줄 lede 작성 (§4.5.4)
오전 11:00 다른 팀원 1명이 publish gate review (§5.2)
정오 12:00 5장 publish + 카톡 공유 trigger + 사이트 hero 시각화 자동 갱신
오후       사용자 반응·공유 monitoring + 정정 요청 (§4.7) review + 다음날 준비
```

**토요일 워크플로우:**
```
오전 7:00  Lead 큐레이터가 한 주 카드 25-30장 review
오전 8:00  가장 활발히 변형된 클레임 1개 선정 (Weekly Flow)
           + 한 주 리더보드 metric 추출 (§4.5.5):
             - 가장 멀리 간 클레임 / 가장 빠르게 변형된 클레임 / 가장 많이 인용된 source
오전 8:30  Weekly Flow 카드 작성 (시간축 + 클레임 변형 timeline + N일째 N차 변형)
오전 9:00  Approver review + publish
```

**Pre-launch backfill 작업 (v3 신규, 5/24~5/29):**
시각화 (§4.5.1) + N일째 카운터 (§4.5.2)가 launch day 빈약하지 않으려면 launch 1주 전부터 시드 데이터 백필 필수.

- 5/24~5/26 (3일): 큐레이터 팀이 4월 말 ~ 5월 중순 정치 YouTube 주요 클레임 20-30개 backfill — 클레임 ID + 시작일 + 변형 history 데이터 모델에 입력
- 5/27~5/29 (3일): backfill된 클레임을 정상 카드 form factor로 publish (publish 시점은 5/27 이후이지만 클레임 시작일은 4월·5월 초)
- 5/30 launch day: hero 시각화에 "추적 중인 클레임 24-30개, 가장 오래된 것 ~30일째" 자연스럽게 노출

**Backfill 운영 부담 (6일 × 큐레이터 팀):** lead Jack ~3시간/일 + approver rotation 적용. burn-out risk → 이 backfill 기간 동안 정상 daily 5장은 4장으로 일시 축소 가능.
...
| Frontend / 개발 | 샘 (Sam Lee, EPFL Computer Science) | **합류 완료, 이번 주부터 함께 작업** |
...
**개발 리딩 (5/5 재훈 콜 결정):**
> 김재훈이 개발 리드. 샘 (이번 주부터 협업) + Saetbyeol (5/11 합류 예정)와의 협업 방식 자율 결정. 재훈이 편한 협업 방식으로, 개발을 리드하면서 두 명을 활용해 같이 진행.

**일정 원칙:**
- AI 자동 생성 일정 (week-by-week 표)에 매이지 않음
- 마일스톤 (5/6 timeline lock): ① 5/6 (화) PRD lock + Lo-Fi 시작, ② 5/7~8 Backend + Hi-Fi, ③ **5/9~10 Demo hard-deadline**, ④ 5/11 (월) Saetbyeol 합류 + 6명 kick-off + 데모 walkthrough, ⑤ **5/16~17 QA wrap-up**, ⑥ 5/30 (토) public launch, ⑦ 6/3 (수) 선거일.
- 5/11~ 부터 6명 다 함께 정기 미팅으로 세부 일정 결정 — AI 자동 생성 일정 폐기.
...
PRD-v2의 marketing/positioning 한 줄. 모든 사용자 facing 페이지에 명시. v3에서는 §4.6 Process Transparency Disclosure 페이지로 한 단계 확장.

**Surface 위치:**
- 사이트 footer
- About 페이지
- "How We Work" 페이지 (v3, §4.6)
...
- **시각화 hero + N일째 카운터 + 분광 메타포 — 흐름 매체 정체성을 매일 visible한 hook으로 구현 (어떤 도구도 안 함)** ← v3 hook layer 추가
...
v2의 week-by-week 풀타임 가정 일정은 폐기 (§5.4). 대신 마일스톤만 명시. 5/6 timeline lock으로 demo hard-deadline (5/9~10) + wrap-up (5/16~17) 단계 명시.

### 7.1 마일스톤 (v3 timeline lock, 2026-05-06)
*Milestones (v3 Timeline Lock, 2026-05-06)*
...
| 시점 | 마일스톤 |
|---|---|
| **2026-05-06 (화)** | PRD v3 lock (hook layer 포함) + Lo-Fi 시작 (Claude Code 데모 빌드 진입) |
| **2026-05-07 (수) ~ 5/8 (목)** | Backend pipeline 시작 (재훈 리드, parallel track) |
| **2026-05-08 (목)** | Hi-Fi 디자인 도달 (카드 form factor + hook layer 시각 디테일) |
| **2026-05-09 (금) ~ 5/10 (토)** | **🔴 Demo hard-deadline.** 디자인 + hook 요소 통합된 작동 데모 완성 |
| **2026-05-10 (토)** | Frontend 통합 + 김현미와 디자인 디테일 1차 점검 |
| **2026-05-11 (월)** | Saetbyeol Leeyouk 합류. 6명 team kick-off + 데모 walkthrough |
| **2026-05-16 (토) ~ 5/17 (일)** | **🔴 QA + final polish.** 데모 wrap-up. Beta 사용자 invite 직전 상태. |
| **2026-05-18 (월) ~ 5/23 (금)** | Beta 사용자 5-10명 invite + feedback 반영 + bug fix |
| **2026-05-24 (토) ~ 5/29 (금)** | Pre-launch backfill (시드 클레임 20-30개, §5.1) + 한국 변호사 preliminary check |
...
**Hard-deadline 2개:**
1. **5/9~10**: 작동 데모 — 디자인 + hook 요소가 실제로 보이고 작동. 이 시점에 분광 메타포 quality bar, A안 vs B안 시각화 등 conditional 결정 final.
2. **5/16~17**: QA wrap-up — beta invite 직전 상태. Backend pipeline + frontend 통합 + 모든 hook layer 작동 검증.

### 7.2 이번 주 협업 흐름 (5/6~5/11, v3 신규)
*This Week's Collaboration Flow*

5/6 Jack lock. 4명 (Jack / 김현미 / 김재훈 / 샘)이 이번 주 협업의 코어. Saetbyeol은 5/11 합류 후 즉시 데모 walkthrough.

```
1. PRD v3 → Jack이 Claude Code로 데모 빌드 (5/6~5/9)
            ↓
2. 데모 → 김현미 디자인 개선 (5/9~5/10, 1차 점검)
            ↓
3. 재훈 + 샘 + Jack 회의 — 개발 논의 (parallel,
   Backend 구조·frontend 통합 방식·기술 stack 결정)
            ↓
4. 기획 논의 필요한 부분 발생 시 →
   김현미 + 재훈 + 샘 + Jack — feature 논의 1회
   (예: 분광 메타포 quality bar 결정, 시각화 A/B 선택,
   영상 썸네일 채택 여부 등)
            ↓
...
**역할 분담 (이번 주):**
- **Jack**: 데모 빌드 lead + 회의 coordination
- **김현미**: 데모 디자인 개선 (스크린샷 정리 + 디테일)
- **김재훈**: Backend pipeline 시작 + 개발 논의 lead
- **샘**: Frontend 협업 + 개발 논의 참여

**회의 trigger:**
- 개발 논의 회의: 재훈/샘/Jack — 데모 1차 빌드 후 (5/8~5/9 사이)
- Feature 논의 회의: 현미/재훈/샘/Jack — 데모 빌드 중 디자인×개발 합의 필요 지점 발생 시 (예: 시각화 A/B 결정 시점)
...
- [ ] 분류 라벨 위반 0건 (지난 1주)
- [ ] 카톡 미리보기 정상 작동 (5개 device 테스트) + "공유 한 줄 hook" 모든 카드 작성됨
...
- [ ] **사이트 hero 시각화 작동 + 시드 클레임 데이터 backfill 완료 (§5.1 pre-launch backfill)** — v3 hook
- [ ] **N일째 카운터 site/card/Weekly Flow 3-layer 모두 작동** (§4.5.2) — v3 hook
- [ ] **분광 메타포 디자인 quality bar 통과 또는 drop 결정** (§4.5.3) — v3 hook
- [ ] **카드 hero 수치 trajectory 시각화 작동** (§4.1) — v3 hook
- [ ] **사이트 hero 한 줄 lede 매일 갱신 워크플로우 정착** (§4.5.4) — v3 hook
- [ ] **Weekly Flow 리더보드 form factor 작동** (§4.5.5) — v3 hook
- [ ] **정정 요청 form 작동 + 24h retraction protocol 연결** (§4.7) — v3 hook
- [ ] (조건부) 영상 썸네일 저작권/캐싱 검토 완료 + 채택/거절 결정

### 7.4 Lo-fi 단계 단축 (v3 신규)
...
2026-05-05 재훈 콜. 사람 주관 개입 위험 (카드 정렬·문구 선택에 큐레이터 사상 들어갈 수 있음)에 대한 답으로 도출. 단순한 "가이드라인 있어요" 수준이 아니라 *절차 자체를 가시화*. Dual purpose: (1) 객관성 추구 의도 implicit signaling (2) 사전 자기 보호 alibi. 자세히는 §4.6.
...
### A.11 왜 샘 (EPFL Computer Science) 합류 (v3 신규)
2026-05-05 재훈 콜. 재훈이 EPFL 컴퓨터 사회사 전공 + 프론트엔드 가능 + 유튜브 영상 분석 경험 있는 샘을 소개. 적극적 의지 보임 (재훈 평가). 이번 주부터 함께 작업. 6번째 멤버 확정으로 v2의 "6번째 TBD" 해소.
...
### A.13 왜 Hook & Felt Pull layer 추가 (v3 신규)
2026-05-06 Jack push back. PRD-v3까지의 결정은 모두 신뢰·정체성 lock에 집중되었고, 이는 1차 타겟 (정치 관심 minority)에는 substance만으로 hook 작동 가정. 그러나 Jack이 "정치 관심 적은 2030 secondary target에게는 매력 부족" 우려를 명시.

핵심 reframing: hook은 "재미"가 아니라 *substance의 surprise + 자기 정체성 신호*. 캐릭터 voice·위트·풍자는 §4.1 톤 가이드라인에 따라 여전히 금지. 흐름 매체 정체성 (§1.3)과 같은 axis 위에 있는 hook만 채택 — 시각화·카운터·메타포·trajectory·리더보드 등.

채택된 hook 8개 (§4.5):
1. 카드 hero 수치 trajectory 제목 + 시각화 (§4.1)
2. 카드 N일째 카운터 + site/Weekly Flow 3-layer (§4.5.2)
3. 사이트 hero "오늘의 흐름" radial spread 시각화 (§4.5.1)
4. PRISM 분광 메타포 design language (조건부 — quality bar) (§4.5.3)
5. 사이트 hero 한 줄 lede (§4.5.4)
6. Weekly Flow 리더보드 (§4.5.5, 사실 metric only)
7. 카톡 미리보기 한 줄 hook (§4.4)
8. 정정 요청 채널 (§4.7)

거절: 제보 채널 (운영 부담 + identity 미스매치, Appendix B.11), 카톡 채널 push (Track 1.5 scope 외, post-launch deferred).

### A.14 왜 분광 메타포 조건부 채택 (v3 신규)
2026-05-06 Jack 결정. 분광 메타포 (PRISM brand의 시각 정체성)는 강력한 hook 후보지만 quality bar에 절대적으로 의존: 색상 design (LGBTQ 함의·정보 시각화 noise·신뢰 톤 검토), 모바일 작동, 디자인 완성도. Jack이 "자칫 완성도 떨어지거나 문제 소지 있게 구현된다면 아예 안 하는 게 낫다"고 명시. 따라서 PRD에 conditional adoption clause를 박아, 데모 review 단계에서 drop 결정이 정치적 부담 없이 가능하도록 함. Drop 시 fallback은 단순 typography 기반 brand identity.
...
재훈이 샘 외 EPFL UI/UX 친구 추가 영입을 제안. Jack 거부 이유: (a) 김현미가 디자인 작업 가능 (b) 7명 팀의 협업 복잡도 증가 (c) 6명 lock으로 운영 안정. 필요 시 재논의.

### B.9 진입 시 modal 강제 (Process Transparency Disclosure 노출 방식, v3 신규 거부)
§4.6 (e) reference. Footer 링크로만. 강제 modal은 사용 마찰.

### B.10 큐레이터 실명 전체 공개 (v3 신규 거부)
§4.6 (c) reference. Jack 1명만 책임자로 실명 노출, 나머지는 역할 단위 익명. 본인 동의 시 사후 추가 공개 가능.

### B.11 사용자 제보 채널 (v3 신규 거부)
v3 hook 검토 단계에서 input loop 후보로 제안. Jack 거부 이유: (a) 제보 받은 후 언론사 전달 process가 현실화 어려움 — Track 1.5 scope 외 (b) PRISM이 "추적"하는 매체이지 "수집"하는 매체가 아니라는 정체성 미스매치. 대신 정정 요청 채널 (§4.7)로 input loop 가치 보존 — 카드 사실 오류 fix는 운영 부담 작고 기존 24h retraction protocol과 직접 연결.

### B.12 카톡 채널 weekly push (v3 — Track 1.5 scope 외, post-launch deferred)
한국 매체 retention의 standard mechanic. 운영 부담 추가 + Track 1.5 scope 초과로 deferred. Post-launch (D+7 이후) 검토 항목.

### B.13 카드 hero에 무지개 색상 (v3 신규 — 5/6 reframing으로 거부 → 조건부 채택으로 전환)
초기 검토 (5/6 오전)에는 좌·우 진영색 (빨강·파랑) 오해 risk로 무지개 거부. Jack push back (5/6 오후): 빨강·파랑이 *둘 다* 포함되면 한쪽 강조가 아니라 diversify된 visual — 오히려 중립. 이 reframing 받아들임. §4.5.1로 색상 design constraint 갱신: 무지개 가능, 단 (a) 한국 LGBTQ 깃발 함의 (b) 정보 시각화 위계 noise (c) 신뢰 매체 톤 — 세 layer를 design 단계에서 검토.

### B.14 사이트 hero 평가 표현 lede (v3 신규 거부)
"오늘 청년 무주택 통계가 35%에서 60%로 부풀려졌습니다" 같은 평가 진술 lede 후보. §4.2 Show-don't-classify 위반 + §4.1 톤 가이드라인 위반. 안전 form: "오늘 청년 무주택 통계 인용이 네 가지 형태로 등장했습니다" 같은 사실 진술만.

### B.15 "이번 주 가장 왜곡된 클레임" 리더보드 ranking (v3 신규 거부)
Weekly Flow 리더보드 검토 단계에서 후보. §4.2 Show-don't-classify 위반 (왜곡 = 평가). 채택된 ranking은 사실 metric only — 가장 멀리 간 / 가장 빠르게 변형된 / 가장 많이 인용된.
...
- **Mitigation**: §4.2 분류 라벨 금지 + §5.2 1 human approver gate + §4.6 Process Transparency Disclosure (legal alibi) + Week 3 한국 변호사 preliminary check + 24시간 내 retraction protocol.
...
- **Risk**: 샘 (이미 합류) + 5/11 Saetbyeol 합류 후 6명 협업 형식 결정에 시간 소요. 개발 리딩이 재훈에게 위임되었지만 협업 방식 자체가 처음.
...
### C.10 Pre-launch backfill 부담 risk (v3 신규 — hook layer)
- **Risk**: 시각화 hero (§4.5.1) + N일째 카운터 (§4.5.2)는 launch day에 충분한 데이터가 있어야 작동. 5/24~5/29 1주 backfill 작업이 정상 daily cadence와 동시 진행 시 burn-out + quality 저하.
- **Mitigation**: backfill 기간 daily cadence 5장 → 4장 일시 축소 허용 + Lead 큐레이터 (Jack) 외 정아인·김현미 backfill 분담 + AI assist 적극 활용 (옛 클레임 자동 추출 후 큐레이터 검수) + Week 3 (5/18~) 일찍 backfill 시작해 시간 분산.

### C.11 분광 메타포 디자인 quality risk (v3 신규 — hook layer)
- **Risk**: 분광 메타포 (§4.5.3) 구현이 디자인 quality 미달 또는 LGBTQ 함의·정보 시각화 noise·신뢰 매체 톤 검토 실패 시, brand identity 약화 + misreading 위험.
- **Mitigation**: Conditional adoption clause (Jack lock) — quality bar 통과 시만 채택, 미달 시 fallback 단순 typography 기반 brand identity. 색상 design 가이드 (§4.5.1, 5/6 갱신) — 무지개 가능 (diversify 메타포), 단 LGBTQ·시각화 noise·신뢰 톤 세 layer 검토 필요. 김현미·Saetbyeol·샘 디자인 리뷰에서 명시적 quality gate.

### C.12 시각화 hook이 분류 도구로 오해될 risk (v3 신규 — hook layer)
- **Risk**: 사이트 hero radial spread 시각화 (§4.5.1)가 "가운데 = 진실, 바깥 = 거짓" 시각 위계로 사용자에게 읽힐 가능성. §4.2 Show-don't-classify 위반 인상.
- **Mitigation**: 시각화 design 원칙 — 중심 = canonical source (정부 1차 출처), 바깥 = YouTube 클레임 (어느 쪽이 맞다 평가 X). 시각화 옆 명시적 한 줄: "PRISM은 분류하지 않습니다. 흐름을 보여드립니다." 디자인 단계에서 이 misreading risk를 김현미·Saetbyeol·샘과 명시적으로 검토.

### C.13 Hook layer overload risk (v3 신규)
- **Risk**: 8개 hook을 한꺼번에 launch에 박으면 카드/사이트가 visually busy → 신뢰 매체 정체성 약화. PRISM의 trustworthy positioning과 충돌.
- **Mitigation**: 5/11 디자인 리뷰에서 hook priority 재검토 — must-have (수치 trajectory + N일째 카운터 + 사이트 hero 시각화 + 카톡 한 줄 hook)와 nice-to-have (분광 메타포 + Weekly Flow 리더보드 + 사이트 hero lede + 정정 요청)를 분리. Quality 보장 안 되는 nice-to-have는 post-launch로 deferred.
...
**Version**: v3.0 (with Hook Layer + Timeline Lock + Week 1 Collaboration Flow)
**Last updated**: 2026-05-06 (NYC, footer simplified + 색상 가이드 갱신 (무지개 조건부 채택) + 운영 layer 정리 + 이번 주 협업 흐름)
**Authors**: Jack Kang (강동재) + Claude (writing assist, all decisions Jack-led)
**Source decisions**: 2026-05-05 [[김재훈]] 콜 (1.5h, KST 14:24~15:53) + 2026-05-05 [[김현미]] 1차 PRD 인계 미팅 (KST 16:00~) + 2026-05-06 hook layer brainstorm + timeline lock
**Demo hard-deadline**: 2026-05-09 ~ 05-10 (다음 작업 — 새 세션에서 Claude Code 데모 빌드 진입)
**Implementation wrap-up**: 2026-05-16 ~ 05-17 (QA + final polish)
**Public launch target**: 2026-05-30 (선거 D-4)
