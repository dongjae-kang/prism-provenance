...
| 이샘 (Sam Lee, EPFL Computer Science) | 미언급 | **이미 active member로 합류** (하루 2-3시간 가용, 이번 주부터 협업 시작) |
...
| 6번째 멤버 | TBD | **이샘(Sam Lee, EPFL CS) 합류로 6명 확정** (4명 active + 이샘 active + 5/11 Saetbyeol 합류 예정) |
...
**색상 design constraint (절대 lock):**
- 무지개 색 금지 (좌·우 진영 색 오해 risk)
- 후보: (a) Monochrome — 흑백 + 회색 그라데이션, (b) 단일 cool tone — 청록·보라·남색 조합, (c) 출처 성격별 색상 mapping (정치인 직접 / 언론 인용 / 2차 가공 — 진영색 회피)
- 김현미 + Saetbyeol + Sam Lee 디자인 단계에서 final 결정
...
| Frontend / 개발 | 이샘 (Sam Lee, EPFL Computer Science) | **이미 active member로 합류** (하루 2-3시간 가용, 이번 주부터 협업) |
...
**개발 리딩 (5/5 재훈 콜 결정):**
> 김재훈이 개발 리드. 이샘 (이미 active member, 이번 주부터 협업) + Saetbyeol (5/11 합류 예정)와의 협업 방식 자율 결정. 재훈이 편한 협업 방식으로, 개발을 리드하면서 두 명을 활용해 같이 진행.

**일정 원칙:**
- AI 자동 생성 일정 (week-by-week 표)에 매이지 않음
- 마일스톤 (5/6 timeline lock): ① 5/6 (화) PRD lock + Lo-Fi 시작, ② 5/7~8 Backend + Hi-Fi, ③ **5/9~10 Demo hard-deadline**, ④ 5/11 (월) Saetbyeol + Sam Lee 합류 + 6명 kick-off + 데모 walkthrough, ⑤ **5/16~17 QA wrap-up**, ⑥ 5/30 (토) public launch, ⑦ 6/3 (수) 선거일.
...
| **2026-05-11 (월)** | Saetbyeol Leeyouk 합류 (이샘은 이미 active). 6명 team kick-off + 데모 walkthrough |
...
5/6 Jack lock. 이샘이 이미 active member로 합류했고, 4명 (Jack / 김현미 / 김재훈 / 이샘)이 이번 주 협업의 코어. Saetbyeol은 5/11 합류 후 즉시 데모 walkthrough.
...
2. 데모 → 김현미 디자인 개선 (5/9~5/10, 1차 점검)
            ↓
3. 재훈 + 이샘 + Jack 회의 — 개발 논의 (parallel,
   Backend 구조·frontend 통합 방식·기술 stack 결정)
            ↓
4. 기획 논의 필요한 부분 발생 시 →
   김현미 + 재훈 + 이샘 + Jack — feature 논의 1회
...
- **이샘**: Frontend 협업 + 개발 논의 참여

**회의 trigger:**
- 개발 논의 회의: 재훈/이샘/Jack — 데모 1차 빌드 후 (5/8~5/9 사이)
- Feature 논의 회의: 현미/재훈/이샘/Jack — 데모 빌드 중 디자인×개발 합의 필요 지점 발생 시 (예: 시각화 A/B 결정 시점)
...
### A.11 왜 이샘 (EPFL Computer Science) 합류 (v3 신규)
2026-05-05 재훈 콜. 재훈이 EPFL 컴퓨터 사회사 전공 + 프론트엔드 가능 + 유튜브 영상 분석 경험 있는 이샘을 소개. 적극적 의지 보임 (재훈 평가). 이미 active member로 합류, 하루 2-3시간 가용, 이번 주부터 협업. 6번째 멤버 확정으로 v2의 "6번째 TBD" 해소.
...
2026-05-06 Jack 결정. 분광 메타포 (PRISM brand의 시각 정체성)는 강력한 hook 후보지만 quality bar에 절대적으로 의존: 색상 design (진영색 회피), 모바일 작동, 디자인 완성도. Jack이 "자칫 완성도 떨어지거나 문제 소지 있게 구현된다면 아예 안 하는 게 낫다"고 명시. 따라서 PRD에 conditional adoption clause를 박아, 데모 review 단계에서 drop 결정이 정치적 부담 없이 가능하도록 함. Drop 시 fallback은 단순 typography 기반 brand identity.
...
### B.13 카드 hero에 무지개 색상 (v3 신규 거부, 절대 lock)
PRISM 분광 메타포의 자연스러운 시각 표현이지만 한국 정치 맥락에서 빨강·파랑 = 진영 색. §1.2 #5 분류 도구가 아니라는 lock과 정면 충돌. 색상 design constraint (§4.5.1): monochrome / 단일 cool tone / 출처 성격별 mapping 중 하나만.
...
- **Risk**: 분광 메타포 (§4.5.3) 구현이 디자인 quality 미달 또는 진영 색상 회피 실패 시, brand identity 약화 + 정치적 misreading 위험.
- **Mitigation**: Conditional adoption clause (Jack lock) — quality bar 통과 시만 채택, 미달 시 fallback 단순 typography 기반 brand identity. 색상 design constraint (§4.5.1) 절대 lock — 무지개 색 금지, monochrome / 단일 cool tone / 출처 성격별 mapping 중 하나만. 김현미·Saetbyeol·Sam Lee 디자인 리뷰에서 명시적 quality gate.
...
- **Mitigation**: 시각화 design 원칙 — 중심 = canonical source (정부 1차 출처), 바깥 = YouTube 클레임 (어느 쪽이 맞다 평가 X). 시각화 옆 명시적 한 줄: "PRISM은 분류하지 않습니다. 흐름을 보여드립니다." 디자인 단계에서 이 misreading risk를 김현미·Saetbyeol·Sam Lee와 명시적으로 검토.
